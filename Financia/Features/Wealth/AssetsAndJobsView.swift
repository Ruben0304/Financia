import SwiftUI

struct AssetsAndJobsView: View {
    @EnvironmentObject private var wealthManager: WealthManager
    @State private var isAddingAsset = false
    @State private var isAddingJob = false

    var body: some View {
        List {
            Section("Activos") {
                if wealthManager.assets.isEmpty {
                    Text("No hay activos registrados.")
                        .foregroundColor(.secondary)
                } else {
                    ForEach(wealthManager.assets) { asset in
                        NavigationLink {
                            AssetDetailView(asset: asset)
                        } label: {
                            HStack {
                                Text(asset.name)
                                Spacer()
                                Text(asset.monthlyEstimates.isEmpty ? "—" : "Estimado")
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                        }
                    }
                    .onDelete { indices in
                        indices.map { wealthManager.assets[$0] }.forEach(wealthManager.deleteAsset)
                    }
                }
            }

            Section("Trabajo") {
                if wealthManager.jobs.isEmpty {
                    Text("No hay trabajos registrados.")
                        .foregroundColor(.secondary)
                } else {
                    ForEach(wealthManager.jobs) { job in
                        NavigationLink {
                            JobDetailView(job: job)
                        } label: {
                            HStack {
                                Text(job.name)
                                Spacer()
                                Text(job.monthlyEstimates.isEmpty ? "—" : "Estimado")
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                        }
                    }
                    .onDelete { indices in
                        indices.map { wealthManager.jobs[$0] }.forEach(wealthManager.deleteJob)
                    }
                }
            }
        }
        .listStyle(.insetGrouped)
        .navigationTitle("Activos y trabajo")
        .navigationBarTitleDisplayMode(.inline)
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Menu {
                    Button("Nuevo activo") { isAddingAsset = true }
                    Button("Nuevo trabajo") { isAddingJob = true }
                } label: {
                    Image(systemName: "plus")
                }
            }
        }
        .sheet(isPresented: $isAddingAsset) {
            AssetEditorView { asset in
                wealthManager.addAsset(asset)
            }
        }
        .sheet(isPresented: $isAddingJob) {
            JobEditorView { job in
                wealthManager.addJob(job)
            }
        }
    }
}

struct AssetEditorView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var name: String = ""
    @State private var notes: String = ""
    @State private var estimates: [MonthlyEstimate] = []

    let onSave: (Asset) -> Void

    var body: some View {
        NavigationStack {
            ZStack {
                DarkFinanceBackground()

                ScrollView(showsIndicators: false) {
                    VStack(spacing: 16) {
                        formCard
                        estimatesCard
                    }
                    .padding(.horizontal, 20)
                    .padding(.vertical, 16)
                }
            }
            .navigationTitle("Nuevo activo")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Cancelar") { dismiss() }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button("Guardar") {
                        let asset = Asset(
                            name: name.trimmingCharacters(in: .whitespacesAndNewlines),
                            notes: notes.trimmingCharacters(in: .whitespacesAndNewlines),
                            monthlyEstimates: estimates
                        )
                        onSave(asset)
                        dismiss()
                    }
                    .disabled(name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                }
            }
        }
    }

    private var formCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            field(label: "Nombre") {
                TextField("Ej. Alquiler, Negocio", text: $name)
                    .foregroundColor(DarkFinanceColors.primaryText)
                    .darkInputStyle()
            }

            field(label: "Notas") {
                TextField("Opcional", text: $notes, axis: .vertical)
                    .foregroundColor(DarkFinanceColors.primaryText)
                    .darkInputStyle()
            }
        }
        .darkFinanceCard(cornerRadius: 16, padding: 16)
    }

    private var estimatesCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Estimado mensual")
                .font(.system(size: 14, weight: .semibold))
                .foregroundColor(DarkFinanceColors.primaryText)

            MonthlyEstimateEditor(estimates: $estimates)
        }
        .darkFinanceCard(cornerRadius: 16, padding: 16)
    }

    private func field<Content: View>(label: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(label)
                .font(.system(size: 13, weight: .medium))
                .foregroundColor(DarkFinanceColors.secondaryText)
            content()
        }
    }
}

struct JobEditorView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var name: String = ""
    @State private var notes: String = ""
    @State private var estimates: [MonthlyEstimate] = []

    let onSave: (Job) -> Void

    var body: some View {
        NavigationStack {
            ZStack {
                DarkFinanceBackground()

                ScrollView(showsIndicators: false) {
                    VStack(spacing: 16) {
                        formCard
                        estimatesCard
                    }
                    .padding(.horizontal, 20)
                    .padding(.vertical, 16)
                }
            }
            .navigationTitle("Nuevo trabajo")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button("Cancelar") { dismiss() }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button("Guardar") {
                        let job = Job(
                            name: name.trimmingCharacters(in: .whitespacesAndNewlines),
                            notes: notes.trimmingCharacters(in: .whitespacesAndNewlines),
                            monthlyEstimates: estimates
                        )
                        onSave(job)
                        dismiss()
                    }
                    .disabled(name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                }
            }
        }
    }

    private var formCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            field(label: "Nombre") {
                TextField("Ej. Empleo, Freelancer", text: $name)
                    .foregroundColor(DarkFinanceColors.primaryText)
                    .darkInputStyle()
            }

            field(label: "Notas") {
                TextField("Opcional", text: $notes, axis: .vertical)
                    .foregroundColor(DarkFinanceColors.primaryText)
                    .darkInputStyle()
            }
        }
        .darkFinanceCard(cornerRadius: 16, padding: 16)
    }

    private var estimatesCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Estimado mensual")
                .font(.system(size: 14, weight: .semibold))
                .foregroundColor(DarkFinanceColors.primaryText)

            MonthlyEstimateEditor(estimates: $estimates)
        }
        .darkFinanceCard(cornerRadius: 16, padding: 16)
    }

    private func field<Content: View>(label: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(label)
                .font(.system(size: 13, weight: .medium))
                .foregroundColor(DarkFinanceColors.secondaryText)
            content()
        }
    }
}
